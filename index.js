...
let totalBackers = GAMES_JSON.reduce((prev, curr) => prev + curr.backers, 0);
// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = totalBackers.toLocaleString("en-US");
// grab the amount raised card, then use reduce() to find the total amount raised
const raisedCard = document.getElementById("total-raised");
let totalPledged = GAMES_JSON.reduce((prev, curr) => prev + curr.pledged, 0);
...
const games = GAMES_JSON.filter((game) => game.pledged < game.goal);
const unfundedCount = games.length;
// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of $${totalPledged.toLocaleString("en-US")} has been raised for ${GAMES_JSON.length} games. Currently, ${unfundedCount} ${unfundedCount == 1 ? "game" : "games"} remains unfunded. We need your help to fund these amazing games!`;
